...
for (int i = 0; i < arr.GetLength(0); i++) // Цикл движения по строкам
{
    ...
    Console.Write($"Сумма элементов в {i+1} строке = {sumLines} "); // Вывожу значения sumLines чтобы убедиться правильно ли работает алгоритм. I + 1 потому, что польлзователь считает с 1 а не с 0
